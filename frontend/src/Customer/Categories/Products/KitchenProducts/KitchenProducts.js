...
const KitchenProducts = ({ wishlistItems, onAddToCart, onToggleWishlist, onViewProduct, cartItems, navigateTo, searchQuery, selectedVendor }) => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                console.log('Fetched Kitchen products:', data);
                    ...
                    <div className="vendor-header">
                        {selectedVendor ? (
                            <div style={{ marginBottom: '20px' }}>
                                <button className="back-button" onClick={() => navigateTo('kitchenproduct')}>
                                    ← Back to Vendors
                                </button>
                                <h2 style={{ color: '#333', fontSize: '2rem', textAlign: 'center' }}>{selectedVendor} - Kitchen Products</h2>
                            </div>
                        ) : (
